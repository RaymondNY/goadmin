    ...
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        <el-form-item label="客户编号">
          <el-input v-model="searchInfo.customerId" placeholder="搜索条件" />
        </el-form-item>
...
export default {
  ...
  // 条件搜索前端看此方法
    ...
    async updateXadLicense(row) {
      const res = await findXadLicense({ ID: row.ID })
      this.type = 'update'
      if (res.code === 0) {
    ...
    async deleteXadLicense(row) {
      const res = await deleteXadLicense({ ID: row.ID })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
