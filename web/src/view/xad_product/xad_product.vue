<template>
  <div>
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        ...
        <el-form-item label="创建人">
          <el-input v-model="searchInfo.createUser" placeholder="搜索条件" />
        </el-form-item>
        <el-form-item>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="硬件信息" prop="hardInfo" width="120" />
        <el-table-column align="left" label="简介" prop="profile" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="text" icon="el-icon-edit" size="small" class="table-button" @click="updateXadProduct(scope.row)">变更</el-button>
...
export default {
  ...
  // 条件搜索前端看此方法
    ...
    async onDelete() {
      ...
      const res = await deleteXadProductByIds({ ids })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
    ...
    async updateXadProduct(row) {
      const res = await findXadProduct({ ID: row.ID })
      this.type = 'update'
      if (res.code === 0) {
    ...
    async deleteXadProduct(row) {
      const res = await deleteXadProduct({ ID: row.ID })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
